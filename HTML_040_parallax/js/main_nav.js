document.addEventListener("DOMContentLoaded", () => {
  ...
  const navClick = (e) => {
    ...
    */
    ...
    if (tagName == "LI") {
      // LI 대문자로 써야함
      let className = e.target.className; // LI 태그로부터 클래스 이름 가져옴

      // className이 art1이면 #art1 을 찾아서 선택
      let art = document.querySelector("#" + className);

      // 그 선택된 박스에 사각형 모양의 바운드(범위)를 가져와라
      // art 객체(Dom 객체)의 현재 위치, 모양값을 getter 해달라
      let bound = art.getBoundingClientRect(); // 여러가지 정보들 들어있음
      // art 객체(box)의 화면의 위에서부터 좌표값 (위에 경계선..)
      let artTop = bound.top;
      // 여러 정보 중에서 top이라는 정보만 참조해서
      window.scrollBy({
        // scrollBy 아직 최신버전에서만 지원
        top: artTop,
        left: 0,
        behavior: "smooth",
      });
  ...
  */
  ...
  const scrollTop_nav_tick = (e) => {
    if (!ticking) {
      ...
      // nav box의 제일 하단 라인에 걸려있는 box tag가 누구인가를 getter하여 ele에 담기
      let art = document.elementFromPoint(0, nav_bottom);

      // 현재 선택된 tag(art에 담겨있는 tag)에서 id값을 추출하여 (ar1,ar2 이런식)
